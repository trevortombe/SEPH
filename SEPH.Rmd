...
seph<-seph %>%
    mutate(test=regexpr(" \\[",NAICS)-1,
         NAICS=ifelse(test>0,substr(NAICS,1,test),NAICS)) %>% data.table()
...
plotdata<-empshare %>%
  ...
  gather(var,value,-Ref_Date)
ggplot(plotdata,aes(Ref_Date,value,group=var,color=var))+
  geom_line()
...
ggplot(plotdata,aes(Ref_Date,TC))+
  ...
  scale_x_yearmon(format="%Y")+
  labs(x="",y="Hours per Week",title="Average weekly hours in Alberta",
       subtitle=paste("Source: Own calculations from ",sourcetable,"-- including overtime; for salaried workers only."),caption="Graph by @trevortombe")
...
plotdata<-vanunemp %>%
  filter(Job.vacancy.statistics=="Unemployment-to-job vacancies ratio",
         grepl("All unemployed",North.American.Industry.Classification.System..NAICS.),
