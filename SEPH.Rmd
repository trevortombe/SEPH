...
ggplot(plotdata %>% filter(Ref_Date>="Jan 2012"),aes(Ref_Date))+
  geom_line(aes(y=avgearn,color="Data"))+
  geom_line(data=plotdata %>% filter(Ref_Date>="Oct 2014"),aes(y=avgearn_fixedhours,color="Fixed Hours"))+
  geom_line(data=plotdata %>% filter(Ref_Date>="Oct 2014"),
            aes(y=avgearn_fixedhrlyshare,color="Fixed Hours and Fixed Emp Share"))
...
#  geom_text(data=plotdata %>% filter(Ref_Date==max(Ref_Date)),show.legend = F,
...
ggplot(plotdata2,aes(Ref_Date,value,group=type,color=type))+
  ...
  geom_text(data=plotdata2 %>% filter(Ref_Date==max(Ref_Date)),show.legend = F,
            aes(label=type),fontface="bold",hjust=0,nudge_x=1/6)+
  mytheme+
  scale_y_continuous(label=dollar,limit=c(NA,1300))+
  scale_x_yearmon(limit=c(NA,max(plotdata$Ref_Date)+3),format="%Y")+
  scale_color_brewer(name="",palette="Set1")+
  labs(y="Dollars per Week",x="",
